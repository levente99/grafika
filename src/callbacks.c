...
void keyboardUp(unsigned char key, int x, int y)
{
	...
	case 'x':
		if (move_chair_id == -1)
		{
			search_closest_chair(&world, camera.position);
		}
		else
		{
			move_chair_id = -1;
	...
	case 'q':
		if (move_chair_id != -1)
		{
			world.chairs[move_chair_id].rotation += 45.0f;
		}
		break;
	case 'e':
		if (move_chair_id != -1)
...
void search_closest_chair(World *world, Vertex camera_position)
{
	int chair_id;
	float min_distance = MAX_MOVING_DISTANCE;
	int i;

	for (i = 0; i < CHAIR_LENGTH; i++)
	{
		float distance = sqrt(pow((camera_position.x - world->chairs[i].position.x), 2) + pow((camera_position.z - world->chairs[i].position.z), 2));
